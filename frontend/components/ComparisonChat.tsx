    ...
    <div className={cn("flex items-start gap-2.5", isUser ? "flex-row-reverse" : "flex-row")}>
      ...
      <div className={cn(
        ...
        {isUser
          ...
          : <Bot className="w-3.5 h-3.5 text-sapphire-400" />}
      </div>
...
export default function ComparisonChat({ comparisonId, doc1Name, doc2Name }: Props) {
  ...
  const send = useCallback(async (content: string) => {
    ...
    // Build history for the API (all prior messages + new user message)
    ...
    <>
      ...
      <button
        ...
        className={cn(
          ...
          open
            ? "bg-ink-800 border border-ink-600 text-ink-300 rotate-0"
      ...
      <div className={cn(
        ...
        {/* Messages */}
        <div className="flex-1 overflow-y-auto bg-ink-950 px-4 py-4 space-y-4 min-h-[320px] max-h-[420px]">
          {messages.length === 0 ? (
        ...
        <div className="px-3 py-3 bg-ink-900 border-t border-ink-800 flex items-end gap-2">
          <textarea
            ...
            onKeyDown={handleKey}
            placeholder="Ask about this comparison… (↵ to send, ⇧↵ for newline)"
            rows={1}
            disabled={streaming}
            className={cn(
