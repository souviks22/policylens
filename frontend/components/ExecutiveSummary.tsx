...
export default function ExecutiveSummary({ result }: Props) {
  ...
  return (
    <div className="space-y-6 tab-content">
      ...
      <div className="p-6 rounded-2xl bg-ink-900 border border-ink-800">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <h3 className="font-serif text-xl font-semibold text-ink-100 mb-1">Executive Summary</h3>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {statCards.map((s) => (
          <div key={s.label} className="p-4 rounded-xl bg-ink-900 border border-ink-800 text-center">
            <div className={`text-2xl font-bold font-mono ${s.color ?? "text-ink-100"}`}>
              {s.value}
            </div>
